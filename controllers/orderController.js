...
//for simple order creating for COD
...
//for creating order online payment using razorpay
export const placedOrderonline = catchAsyncError(async (req, res, next) => {
  const {
    shippingInfo,
    orderItems,
    paymentMethod,
...
//catchAsyncError is a replacement for try catch block
export const paymentVerification = catchAsyncError(async (req, res, next) => {
  ...
  //the sha256 is algorithm of the razorpay
  const expectedSignature = crypto
    .createHmac("sha256", process.env.RAZORPAY_API_SECRET_KEY)
    .update(body)
    .digest("hex");

  const isAuthenticPay = expectedSignature === razorpay_signature;
  ...
  if (isAuthenticPay) {
    ...
    res.status(201).json({
      sucess: true,
      message: `Order Placed Sucessfully, your Payment ID is ${payment._id}`,
    });
  } else {
